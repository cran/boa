...
#-------------------------------------------------------------------------------
{
   recover <- recover && exists(".boa.par", env = globalenv()) &&
      exists(".boa.par", env = globalenv()) &&
      exists(".boa.pardesc", env = globalenv()) &&
      exists(".boa.chain", env = globalenv()) &&
      exists(".boa.version", env = globalenv())
      ...
      assign(".boa.pardesc",
            structure(rbind(
               c("Convergence", "Brooks, Gelman, & Rubin", "Alpha Level", "alpha",
                  ""),
               c("Convergence", "Brooks, Gelman, & Rubin", "Window Fraction",
                  "gandr.win", ""),
